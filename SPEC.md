...
**Status:** V1 built (M1–M5) · Phase 2 built (M6–M9) · M10 owner-review round done · M11 admin rebuild + M12 student rebuild directed (§15.6/§15.7 #23–#24) · **Name:** "Road to Success by Anglou Dimitra", formerly the working title Lumen (§15.7 #20) · **Last updated:** 2026-08-24
...
| 23 | §7/§15.5 admin IA: eight nav items (Students home, Requests, Courses, Modules, Progress, Calendar, Messages, Settings); modules created from a global list behind a cohort dropdown | Admin rebuild (M11) per the approved `design/admin-blend-final.html` | **APPROVED (owner, 2026-08-24):** the admin is course-centred. Nav is exactly **Courses (home) · Students · Messages (unread count) · Calendar · Settings** plus the initials chip and sign out. `/admin` is the courses home: a "Needs you" strip rendered only when something truly needs her (pending requests with inline approve/decline, an unread-messages line, a listed course with zero modules, and the content nudge for a module releasing within 7 days that is missing videos, slides, exercises or solutions), then course cards (active count, request chip, listed chip, last release with submitted count, next release) and a dashed New-course card opening the create form at `/admin/courses/new`. `/admin/courses/[id]` is the course workspace with tabs **Modules** (week rail with completeness labels + the inline "Add week N" composer: week autofilled, dd/mm/yyyy day, fixed 09:00 Athens, no cohort dropdown) **· Students** (this course's enrollments with submitted x of y, pause/end, add a student) **· Progress** (the matrix scoped to the course) **· Details** (blurb, listed, subject, level, exam year). `/admin/modules/[id]` keeps its URL, rebuilt as the four-slot week editor: Videos (multi-file, drag to reorder, per-file remove, processing copy "You can leave this page, it finishes on its own"), Slides / Exercises / Solutions (single file with Replace; the solutions slot carries the Rule 2 lock note), a "Ready for Monday?" checklist (driven by `lib/content.ts`) with a one-line summary, the weekly-note textarea autosaving with a "Saved just now" indicator, and "Preview as a student" (`/admin/modules/[id]/preview`, an admin-shell render of the student view). `/admin/students` hosts the approved direction C drawer: calm rows (initials, name, course summary, request chip, progress bar, last seen) opening a full-screen sheet at 390 with enrollment controls, approve/decline where pending, add to course, set password, the global pause (Lever 1 stays reachable), "Open message thread", and a one-line history; the create-account form sits behind a dashed row. **Retired routes:** `/admin/modules`, `/admin/requests`, `/admin/progress` redirect to `/admin` (every action they hosted reachable in its new home first); the old `/admin/courses` list also redirects to `/admin`, its create form now at `/admin/courses/new`, its blurb/listed editing in the Details tab. Recorded assumption: the week editor keeps a compact "Week details" card (title, week number, release day) so those edits stay reachable, the mockup not showing them notwithstanding |
| 24 | §15.4 student IA: seven nav items; `/app` was the dashboard (hero, this-week card, cross-course rail); Sessions and Calendar were separate pages | Student rebuild (M12) per the approved `design/student-direction-2-course-hub.html` | **APPROVED (owner, 2026-08-24):** the student app mirrors the admin. Nav is **Courses · Messages · Schedule**, with Account behind the initials chip; the RTS monogram below 640 stays. `/app` is **My courses**: one card per enrollment (active and paused states) with progress bar, "Week N of M", a "new this week" line, and a Continue button deep-linking into that course's current module; the catalog with "Ask to join" (the old `/app/courses` page) sits below the cards. `/app/courses/[id]` is the course hub: the weekly note card for the latest released week on top, then the course-scoped week rail (done with Review, current with the single orange Continue, locked with unlock dates). `/app/schedule` merges Sessions and Calendar: the month grid / agenda from `lib/calendar.ts`, the weekly clinic line, and the "Book a 1:1 on Google Meet" button. **Retired:** the old dashboard and `/app/assignments` (redirects to `/app`); `/app/courses` redirects to `/app`; `/app/sessions` and `/app/calendar` redirect to `/app/schedule`. Messages and Account are unchanged. Flagged conflict, resolved by the directive's verify line ("each Continue lands in the right course's current module") over the mockup (which shows Continue on one card): **every active course card with a released module carries Continue**, and the one-orange rule (#13) is kept by rendering only the overall current course's Continue as the orange CTA, the others as primary blue |
| 23 | §7/§15.5 admin IA: nav was Students (home) · Requests · Courses · Modules · Progress · Calendar · Messages · Settings; the module editor was a flat form plus a typed dropzone | Owner rebuild (M11); `design/admin-blend-final.html` is authoritative | **APPROVED (owner, 2026-08-24):** admin nav is exactly **Courses (home at /admin) · Students · Messages (unread count) · Calendar · Settings** plus the initials chip and sign out. /admin = the "Needs you" strip, rendered only when something truly needs her (pending join requests with inline Approve and Decline; an unread-messages line; a listed course with zero modules; a content nudge for any module releasing within the next 7 days that is missing videos, slides, exercises or solutions) + course cards + a dashed New course card (create form on its own page, /admin/courses/new). /admin/courses/[id] = tabs **Modules** (week rail jade/blue/linen, per-row completeness and submitted counts, inline "Add week N" composer with the week autofilled, dd/mm/yyyy release day at a fixed 09:00 Athens, no cohort dropdown) · **Students** (this course's enrollments, Pause/End, add a student) · **Progress** (the matrix scoped to the course) · **Details** (blurb, listed, subject, level, exam year). The week editor becomes four labeled slots (Videos: multi-file, drag to reorder, per-file remove, processing copy "You can leave this page, it finishes on its own"; Slides, Exercises, Solutions: single file with Replace; the solutions slot carries the lock note) with a "Ready for Monday?" checklist + one-line summary, the autosaving weekly note ("Saved just now"), and a "Preview as a student" link. Uploads stay on the existing storage path. **Retired routes:** /admin/modules, /admin/requests and /admin/progress redirect to /admin, with every action they hosted reachable in its new home first; the old /admin/courses list redirects to /admin |
| 24 | §15.4 student IA: nav was Home · Courses · Assignments · Messages · Sessions · Calendar · Account; /app was the dashboard | Owner rebuild (M12); `design/student-direction-2-course-hub.html` is authoritative | **APPROVED (owner, 2026-08-24):** student nav is **Courses · Messages · Schedule** plus the chip (Account lives behind the chip); the RTS monogram below 640px stays. /app = **My courses**: one card per enrollment (active and paused states) with the progress bar, "Week N of M", the new-this-week line, and a Continue button deep-linking into the current module (single-course students pay no extra tap); the catalog with Ask to join sits below (the old /app/courses promoted to home). /app/courses/[id] = the weekly note card for the latest released week, then the week rail (done with Review, current with the single orange Continue, locked with unlock dates), scoped to that course. /app/schedule merges Sessions and Calendar: the grid or agenda from lib/calendar.ts, the weekly clinic line, and the "Book a 1:1 on Google Meet" button. **Retired routes:** the old dashboard content and /app/assignments retire; /app/courses redirects to /app; /app/sessions and /app/calendar redirect to /app/schedule. Messages and Account are unchanged |
