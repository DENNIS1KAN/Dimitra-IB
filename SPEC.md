...
| 11 | §15.4 `/app/account`: "change own password (current + new, min 8, same scrypt path)" says nothing about existing sessions | — | Assumption (M8): a successful change **signs out the student's other devices** (their other session rows are deleted; the current one stays). Standard hygiene; one line to remove if unwanted |
| 12 | §15.3 `booking_url` has no format rule | — | Assumption (M8): must be an absolute `http(s)://` URL or empty (a `javascript:` value would become a live new-tab link); `clinic_text` is trimmed and capped at 2000 characters |
