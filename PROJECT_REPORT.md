...
*A complete technical report on what has been built and how it works, regenerated strictly from the repository at HEAD on 2026-08-22 after Phase 2 (M6–M9). Written to be handed to an AI assistant or a new developer as full project context. The remaining work before real students use it is in `LAUNCH.md`.*
...
- **Sources of truth:** `SPEC.md` (scope/behavior; §15 holds the owner's Phase 2 amendments, milestones M6–M9 with their verified checklists, and the decision log §15.7), `DESIGN.md` (Dimitra's palette, tokens, component and screen recipes), `CLAUDE.md` (working rules). The gating rules in `lib/gating.ts` are *the entire business logic*.
...
4. **Soft deadline** (`isOverdue(dueDate, hasSubmission, now)`, SPEC §15.1): a badge only — submission is never blocked; submitting clears it.

`lib/current.ts` picks "this week's" module: most recent release → nearest due date → course title A–Z → higher week number (SPEC §15.7 #9).
...
- **`modules`** — cohort, week number (unique per cohort), title, description (the weekly note), `release_date`, nullable `due_date` (the admin form defaults it to the Sunday 23:59 after release, tutor timezone — `defaultDueLocal`).
...
The blue nav bar (`app/app/layout.tsx`, mounted once) carries Home · Courses · Assignments · Messages (unread dot) · Sessions · Account; under 1024px the links wrap to a scrollable row. Rule 3 lives in this layout.

- **`/app`** — the dashboard per `design/lumen-dashboard-mockup.html`: indigo hero (course names, greeting by the tutor's clock, jade progress), the note from Dimitra (the current module's description), the "This week" feature card with the view's one orange CTA (due date, Overdue badge), the clinic strip from `settings.clinic_text`, the term rail (current · older released · locked), footer. With several active courses rows name their course; a single-course student sees one course.
- **`/app/modules/[id]`** — back link, module title, badge row (New this week / course / Attempt sent / Overdue) and soft-deadline caption, LessonRows (videos → watch page; slides inline + stamped download; exercises download), then the solutions block: locked panel whose "Submit my attempt" (the page's orange CTA) opens the bottom sheet (photo/PDF ≤ 25MB, note, or "just mark as attempted"), or the unlocked panel + solutions row.
...
- **`/app/assignments`** — every open module across enrollments by due date with overdue badges, then completed.
...
- **`/admin`** — students: name/email, username, per-course enrollments (Pause / Resume / End, add-to-course), global active toggle, last seen, password reset; account creation (the chosen cohort becomes the first active enrollment, in one transaction).
...
- **`/admin/modules`**, **`/admin/modules/[id]`** — modules by cohort with release and due badges; create/edit (week, release + due pair with the Sunday default, title, description); drag-and-drop material upload, rename, reorder, delete.
- **`/admin/progress`** — students × released modules from enrollments; a submitted cell links to **`/admin/submissions/[id]`** (note + inline image / PDF) served by the admin-only `/api/admin/submissions/[id]/file` (404 to anyone else).
...
docs/superpowers/plans/                     # per-milestone implementation plans (M6–M9)
